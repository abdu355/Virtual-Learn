...
namespace UnityStandardAssets.Characters.FirstPerson
{
	...
	public class RigidbodyFirstPersonController : MonoBehaviour
	{
		...
		public AudioClip m_FootstepSounds1,m_FootstepSounds2,m_FootstepSounds3;
		public AudioSource m_AudioSource;
		public bool incuberoom, inforest, inhouse;
		...
		private void PlayFootStepAudio (Vector2 input)//mycode
		{
			if (!m_IsGrounded) {
				return;
			}
			if (input.x == 0 && input.y == 0) {
				if (m_AudioSource.isPlaying)
					m_AudioSource.Stop ();
			} else if (input.x > 0 || input.y > 0 || input.x<0 || input.y<0) {
				// pick & play a random footstep sound from the array,
				// excluding sound at index 0
				//int n = Random.Range (0, m_FootstepSounds.Length);
				if (incuberoom) {
					//m_AudioSource.clip = m_FootstepSounds1;
				} else if (inforest) {
					//m_AudioSource.clip = m_FootstepSounds2;
				} else if (inhouse) {
					//m_AudioSource.clip = m_FootstepSounds3;
				}
				if (!m_AudioSource.isPlaying)
		...
		private void FixedUpdate ()
		{
			...
			//PlayFootStepAudio (input); //mycode
			if (isLook) {
